...
const TOOL_ITEMS = new Set(["commandExecution", "webSearch", "fileChange", "mcpToolCall", "collabAgentToolCall", "subAgentActivity", "imageView", "sleep"]);
...
function readRollout(file) {
  ...
  // Streaming rollouts can replay one response as it is updated. Keep only its
  // most complete record before computing any aggregate.
  const byResponse = new Map(), anonymous = [];
  for (const r of recs) {
    if (r.id == null) { anonymous.push(r); continue; }
    const prev = byResponse.get(r.id);
    const total = r.u.total_tokens ?? ((r.u.input_tokens || 0) + (r.u.output_tokens || 0));
    const prior = prev ? (prev.u.total_tokens ?? ((prev.u.input_tokens || 0) + (prev.u.output_tokens || 0))) : -1;
    if (!prev || total >= prior) byResponse.set(r.id, r);
  }
  recs = [...byResponse.values(), ...anonymous];
...
/** Recognise a rollout before Claude's broad JSONL path adapter can claim it. */
export function refFromPath(file) {
  const state = statePath();
  if (!fs.existsSync(state)) return null;
  const h = open(state), abs = path.resolve(file);
  const threads = rows(h, "select id, rollout_path, cwd, created_at, updated_at, updated_at_ms from threads") || [];
  for (const t of threads) if (resolveRollout(t.rollout_path) === abs) {
    const p = resolveRollout(t.rollout_path); let size = 0;
    try { size = fs.statSync(p).size; } catch { /* live/missing */ }
    return { provider: name, id: t.id, project: String(t.cwd ?? "").replace(/\//g, "-"), path: p,
      mtimeMs: t.updated_at_ms ?? t.updated_at * 1000, size };
  }
  return null;
}
...
function synth(t, roll, createdMs, updatedMs, hist) {
  ...
  const items = hist
    ...
    : [];
  // Realtime records are not projected onto a durable turn by Codex. They are
  // still observed and classified, but never fabricated as a response action.
  const realtime = hist
    ? rows(hist, "select item_type, count(*) c from thread_realtime_items where thread_id = ? group by item_type", t.id) || []
    : [];
  ...
  for (const r of realtime) noteItemType(r.item_type);
  ...
  const calls = (roll ? roll.calls : []).map(k => {
    ...
    return { id: k.id, ts: k.ts, cost: k.cost, model: k.model,
             ...
             // History only proves activity at turn granularity. Retain that
             // evidence on calls without pretending it identifies one response.
             acts: Object.entries(tools.get(k.turnId) || {}).map(([name, count]) => ({ name, count, id: null, skill: null, task: null })),
