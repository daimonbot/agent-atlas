...
function SortHeader({ label, field, filters, onChange, numeric = false }) { const active = filters.sort === field; return <th className={"s col-" + field + " " + (numeric ? "r" : "")} onClick={() => onChange({ ...filters, sort: field, dir: active && filters.dir === "desc" ? "asc" : "desc" })}>{label}<span className="arr">{active ? filters.dir === "desc" ? "▼" : "▲" : "⇅"}</span></th>; }
function SessionRow({ row }) { const token = row.tokens || {}, tokenCost = row.tokenCost || {}, workspace = row.workspace || {}; const metric = key => <td className={"r col-" + key + " " + (!token[key] ? "zero" : "")} key={key}>{token[key] ? <>{tokenCount(token[key])}<span className="usd">{money(tokenCost[key])}</span></> : "·"}</td>; return <tr><td className="l col-live">{row.live ? <span className="badge live">LIVE</span> : <span className="badge done">closed</span>}</td><td className="l ag col-title"><div className="agl"><a href={linkTo("/session/" + row.id)}>{row.title || row.name || row.project}</a></div><div className="agd"><span className="model-label" title="Model">{row.model || "unknown model"}</span><span className="mono">{row.id.slice(0, 8)}</span>{row.subtitle || row.desc ? " · " + (row.subtitle || row.desc) : ""}</div></td><td className="l proj col-project" title={workspace.cwd || ""}><div className="m1">{workspace.label || row.project}</div><div className="m2">{workspace.branch ? "⎇ " + workspace.branch : ""}</div></td><td className="dim mono col-start">{shortDate(row.start)}</td><td className="r dim col-duration">{duration(row.durationS)}</td><td className="r dim col-agentMs">{row.agentMs ? duration(Math.round(row.agentMs / 1000)) : "·"}</td><td className="r dim col-agents">{num(row.agents)}</td><td className="r dim col-calls">{num(row.apiCalls)}</td><td className="r dim col-humanMsgs">{num(row.humanMsgs)}{row.costPerHumanMsg != null && <span className="usd">{money(row.costPerHumanMsg)}/msg</span>}</td>{["in", "out", "cr", "cw"].map(metric)}<td className="r money col-cost">{money(row.cost)}</td></tr>; }
function VirtualSessionTable({ filters, onChange, data }) {
  const queryClient = useQueryClient(); const [scrollMargin, setScrollMargin] = useState(0); const tableRef = React.useCallback(node => { if (node) setScrollMargin(node.offsetTop); }, []); const pageSize = 100;
  const virtual = useWindowVirtualizer({ count: data?.matchedCount || 0, estimateSize: () => 58, overscan: 12, scrollMargin });
  const virtualItems = virtual.getVirtualItems();
  const offsets = [...new Set((virtualItems.length ? virtualItems : [{ index: 0 }]).map(item => Math.floor(item.index / pageSize) * pageSize))];
  const pages = useQueries({ queries: offsets.map(offset => ({ queryKey: ["sessions-range", filters, data?.revision, offset], enabled: offset !== 0, queryFn: () => api("/api/ui/sessions", sessionParams(filters, { offset, limit: pageSize, revision: data?.revision })) })) });
  const pageRows = new Map(); data.rows?.forEach((row, index) => pageRows.set(index, row)); pages.forEach((page, pageIndex) => page.data?.rows.forEach((row, index) => pageRows.set(offsets[pageIndex] + index, row)));
  const first = virtualItems[0]?.index || 0, last = virtualItems.at(-1)?.index ?? 0, currentPage = Math.floor(first / pageSize), pageStart = currentPage * pageSize;
  useEffect(() => { [pageStart - pageSize, pageStart + pageSize].filter(offset => offset > 0 && offset < data.matchedCount).forEach(offset => queryClient.prefetchQuery({ queryKey: ["sessions-range", filters, data.revision, offset], queryFn: () => api("/api/ui/sessions", sessionParams(filters, { offset, limit: pageSize, revision: data.revision })) })); }, [data.matchedCount, data.revision, filters, pageStart, queryClient]);
  const top = Math.max(0, (virtualItems[0]?.start || 0) - scrollMargin), bottom = Math.max(0, virtual.getTotalSize() - (virtualItems.at(-1)?.end || 0));
  const goToPage = page => virtual.scrollToIndex(Math.max(0, Math.min(data.matchedCount - 1, page * pageSize)), { align: "start" });
  const loading = pages.some(page => page.isLoading);
  return <section className="sessions-table" aria-label="Sessions"><div className="table-meta" role="status"><span>{data.matchedCount ? `Sessions ${first + 1}–${Math.min(last + 1, data.matchedCount)} of ${num(data.matchedCount)}` : "No sessions match these filters"}</span><span className="table-meta-note">{loading ? "Loading rows…" : "Rows load in 100-session chunks"}</span><nav aria-label="Session pagination"><button disabled={currentPage === 0} onClick={() => goToPage(currentPage - 1)}>Previous 100</button><button disabled={pageStart + pageSize >= data.matchedCount} onClick={() => goToPage(currentPage + 1)}>Next 100</button></nav></div><div className="sessions-table-region"><table className="tt" ref={tableRef}><thead><tr><SortHeader label="status" field="live" filters={filters} onChange={onChange} /><SortHeader label="session" field="title" filters={filters} onChange={onChange} /><SortHeader label="project" field="project" filters={filters} onChange={onChange} /><SortHeader label="started" field="start" filters={filters} onChange={onChange} /><SortHeader label="dur" field="duration" filters={filters} onChange={onChange} numeric /><SortHeader label="agent time" field="agentMs" filters={filters} onChange={onChange} numeric /><SortHeader label="agents" field="agents" filters={filters} onChange={onChange} numeric /><SortHeader label="calls" field="calls" filters={filters} onChange={onChange} numeric /><SortHeader label="human messages" field="humanMsgs" filters={filters} onChange={onChange} numeric /><SortHeader label="in" field="in" filters={filters} onChange={onChange} numeric /><SortHeader label="out" field="out" filters={filters} onChange={onChange} numeric /><SortHeader label="cache read" field="cr" filters={filters} onChange={onChange} numeric /><SortHeader label="cache write" field="cw" filters={filters} onChange={onChange} numeric /><SortHeader label="cost" field="cost" filters={filters} onChange={onChange} numeric /></tr></thead><tbody>{top > 0 && <tr aria-hidden="true"><td colSpan={14} style={{ height: top, padding: 0 }} /></tr>}{virtualItems.map(item => { const row = pageRows.get(item.index); return row ? <SessionRow key={row.id} row={row} /> : <tr key={item.index} className="skeleton"><td colSpan={14}>Loading session…</td></tr>; })}{bottom > 0 && <tr aria-hidden="true"><td colSpan={14} style={{ height: bottom, padding: 0 }} /></tr>}</tbody></table></div></section>;
}
